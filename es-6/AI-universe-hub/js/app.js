...
const aiToolsDataLoad = async () => {
    try {
        ...
        newArray = datas;
        const dataSlice = datas.slice(0,6);
        displayAiTools(dataSlice);
    } catch (error) {
        console.log(error);
    }
}
    ...
    try {
        ...
        const { score } = accuracy;
            ...
            ${score * 100}% accuracy
                ...
                document.getElementById('basicPlan').innerHTML = `${pricing ? pricing[0].price + '<br>' + pricing[0].plan : 'Free of Cost/Basic'}`;
                document.getElementById('proPlan').innerHTML = `${pricing ? pricing[1].price + '<br>' + pricing[1].plan : 'Free Of Cost/Pro'}`;
                document.getElementById('diamond').innerHTML = `${pricing ? pricing[2].price + '<br>' + pricing[2].plan : 'Free of Cost /Enterprise'}`;
